...
def main():
    ...
    parser.add_argument('--quiet', action='store_true', help='Suppress per-sample logging')
    
    args = parser.parse_args()
    
    def log(*values: Any, **kwargs: Any) -> None:
        print(*values, **kwargs)

    def trace(*values: Any, **kwargs: Any) -> None:
        if not args.quiet:
            print(*values, **kwargs)

    log('='*70)
    log('EVALUATING MODEL.PY')
    log('='*70)
    ...
    # Load dataset
    log(f'Loading dataset: {args.data_dir}')
    ...
    # Load model
    log(f'Loading model: {args.pretrained_model}')
    model = MultiHeadKoBART.from_pretrained(args.pretrained_model)
    
    # Load checkpoint
    log(f'Loading checkpoint: {args.ckpt_path}')
    ...
    with torch.no_grad():
        for idx_num, idx in enumerate(selected, 1):
            ...
            if can_print:
                trace(f'[{idx_num}/{len(selected)}] Task: {task} | Loss: {loss:.4f}')
                trace(f'Input:  {stripped}')
                trace(f'Target: {tgt_text}')
                trace(f'Pred:   {pred_text}')
                trace()
    ...
    log('='*70)
    log(f'Total samples: {len(selected)}')
    log(f'Average loss:  {total_loss / len(selected) if selected else 0.0:.4f}')
    log()
    log('Per-task results:')
    for task in sorted(task_losses.keys()):
        losses = task_losses[task]
        avg_loss = sum(losses) / len(losses) if losses else 0.0
        log(f'  {task:24s}: {len(losses):3d} samples, avg loss: {avg_loss:.4f}')
