...
- Optional train/validation split with per-task oversampling factors and deterministic sampling via seed control.
...
Usage:
	...
	dl = build_dataloader(ds, sampler=sampler)
...
class MultiTaskDataset(Dataset):
	...
	def __init__(
		...
		split: str = 'train',
		validation_split: float = 1.0 / 11.0,
		...
		self.seed = seed
		self.task_repeat_factors = task_repeat_factors or {}
		...
		for task in self.include_tasks:
			...
			loaded = self._load_jsonl(fpath, max_samples=max_samples, skip_samples=skip_samples)
			selected = self._select_split_samples(task, loaded)
			if self.split == 'train':
				selected = self._apply_repeat_factor(task, selected)
			self.data[task] = selected
			meta = []
			if skip_samples:
				meta.append(f"skipped first {skip_samples}")
			if max_samples and max_samples < len(loaded):
				meta.append(f"capped at {max_samples}")
			if self.split in ('train', 'validation') and 0 < self.validation_split < 1:
				split_pct = 100.0 * (1.0 - self.validation_split if self.split == 'train' else self.validation_split)
				meta.append(f"{self.split} split ~{split_pct:.1f}%")
			repeat = self.task_repeat_factors.get(task)
			if self.split == 'train' and repeat and repeat > 1.0:
				meta.append(f"repeat x{repeat}")
			meta_str = f" ({', '.join(meta)})" if meta else ''
			print(f"Loaded {len(self.data[task])} samples for task '{task}' from {fpath.name}{meta_str}")
		...
		for task, items in self.data.items():
			...
			self._rng.shuffle(self.examples)
	...
	def _select_split_samples(self, task: str, records: List[Dict]) -> List[Dict]:
		if not records:
			return []
		# Allow bypass when consuming all data (e.g., evaluation)
		if self.split not in ('train', 'validation') or not (0.0 < self.validation_split < 1.0):
			return list(records)
		if len(records) <= 1:
			if self.split == 'validation':
				# For single-sample tasks, reserve for training only
				return []
			return list(records)
		task_rng = random.Random(self.seed + hash(task) % (2**31))
		indices = list(range(len(records)))
		task_rng.shuffle(indices)
		val_count = int(round(len(records) * self.validation_split))
		if val_count <= 0:
			val_count = 1
		...
		if self.split == 'validation':
			return [records[i] for i in indices if i in val_idx]
		return [records[i] for i in indices if i not in val_idx]

	def _apply_repeat_factor(self, task: str, records: List[Dict]) -> List[Dict]:
		if not records:
			return []
		repeat = float(self.task_repeat_factors.get(task, 1.0))
		if repeat <= 1.0:
			return list(records)
		int_repeat = int(repeat)
		remainder = repeat - int_repeat
		result: List[Dict] = []
		if int_repeat > 0:
			result.extend(records * int_repeat)
		if remainder > 0.0:
			extra = max(1, int(round(len(records) * remainder)))
			choice_rng = random.Random(self.seed + 17 + hash(task) % (2**31))
			result.extend(choice_rng.choices(records, k=extra))
...
class TaskBatchSampler(Sampler[List[int]]):
	...
	def __init__(self, dataset: MultiTaskDataset, batch_size: int, shuffle: bool = True, drop_last: bool = True) -> None:
		...
		for t, lst in self.by_task.items():
			for i in range(0, len(lst), self.batch_size):
				...
				if len(chunk) == self.batch_size or (not self.drop_last and len(chunk) > 0):
...
def build_dataloader(
	...
	if sampler is None:
		sampler = TaskBatchSampler(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last)
...
if __name__ == '__main__':
	...
	p.add_argument('--split', type=str, default='train', choices=['train', 'validation', 'all'])
	p.add_argument('--validation_split', type=float, default=1.0 / 11.0)
	p.add_argument('--task_repeat_factors', type=str, default=None, help='task:factor pairs for oversampling (train split only)')
	args = p.parse_args()
