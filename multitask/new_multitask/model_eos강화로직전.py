    ...
    def generate(
        ...
        **kwargs
        ...
        for step in range(max_length - 1):
            ...
            # Super aggressive EOS token boosting
            current_len = dec_in.size(1)
            for i in range(bsz):
                if not is_finished[i]:
                    adaptive_max = adaptive_max_lengths[i].item()
                    
                    # Start EOS boosting much earlier (40% of adaptive max)
                    if current_len >= adaptive_max * 0.4:
                        # Exponential EOS boosting
                        progress = (current_len - adaptive_max * 0.4) / (adaptive_max * 0.6)
                        # Exponential boost: up to 50x at the end
                        boost_factor = 1.0 + progress ** 2 * 50.0
                        logits[i, self.config.eos_token_id] = logits[i, self.config.eos_token_id] * boost_factor
                    
                    # Very aggressive force at 80% of adaptive max
                    if current_len >= adaptive_max * 0.8:
                        # Suppress all other tokens significantly
                        logits[i, :] = logits[i, :] - 5.0
                        # Boost EOS extremely
                        logits[i, self.config.eos_token_id] = logits[i, self.config.eos_token_id] + 10.0
